...
class PortfolioApp {
    ...
    loadDynamicMedia() {
        const portfolioMedia = this.getPortfolioMedia();
        
        Object.keys(portfolioMedia).forEach(section => {
            this.addDynamicMediaToSection(section, portfolioMedia[section]);
        });
    }
    ...
    addDynamicMediaToSection(sectionId, mediaItems) {
        ...
        mediaItems.forEach(mediaItem => {
            if (!this.mediaItemExists(mediaGallery, mediaItem.id)) {
    ...
    setupAdminEventListeners() {
        // Listen for media upload events from admin panel
        window.addEventListener('mediaUploaded', (e) => {
            const { section, mediaItem } = e.detail;
            this.addDynamicMediaToSection(section, [mediaItem]);
            this.showNotification('새 미디어가 추가되었습니다!', 'success');
        });

        // Listen for media deletion events from admin panel
        window.addEventListener('mediaDeleted', (e) => {
            const { section, itemId } = e.detail;
            this.removeDynamicMediaItem(section, itemId);
            this.showNotification('미디어가 삭제되었습니다.', 'info');
        });
    }
